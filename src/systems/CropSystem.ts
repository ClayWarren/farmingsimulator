...
export class CropSystem {
    ...
    private createCropTemplates(): void {
        Object.entries(this.cropInfo).forEach(([type, info]) => {
            ...
            const template = MeshBuilder.CreateCylinder(`${cropType}_template`, {
                height: 0.1,
                diameterTop: 0.8,
                diameterBottom: 0.2,
                tessellation: 6
    ...
    plantCrop(cropType: CropType, position: Vector3): boolean {
        ...
        console.log(`Planted ${cropType} at position (${position.x}, ${position.z})`);
        return true;
    }
    ...
    private calculateDaysSincePlanted(crop: CropData, currentTime: TimeData): number {
        if (crop.plantedSeason === currentTime.season) {
            return currentTime.day - crop.plantedDay;
        }
        
        const seasonsToAdd = this.getSeasonDifference(crop.plantedSeason, currentTime.season);
        return (seasonsToAdd * 30) + currentTime.day - crop.plantedDay;
    }

    private getSeasonDifference(plantedSeason: string, currentSeason: string): number {
    ...
    getMatureCropCount(): number {
        return Array.from(this.crops.values()).filter(crop => crop.growthStage === 3).length;
    }
}